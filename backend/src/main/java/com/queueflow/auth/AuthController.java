...
public class AuthController {
    ...
    public AuthController(AuthService auth, @Value("${app.security.cookie-secure:false}") boolean secureCookie) {
        this.auth = auth;
    ...
    private ResponseEntity<AuthResponse> respond(AuthService.Session session, HttpServletResponse response) {
        response.addHeader(HttpHeaders.SET_COOKIE, cookie(session.refreshToken(), 14 * 86400).toString());
